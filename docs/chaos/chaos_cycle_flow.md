...
- **包含数据**:
    - **账户信息**: 权益、余额、未实现盈亏。
    - **持仓快照**: 当前所有持仓的详细信息。
    - **市场数据**: 候选币种的 K 线、指标数据 (调用 `market.GetWithTimeframes`)。
    - **排行数据**: OI Top、涨跌幅排行等。
...
- **关键调用**: `e.manager.ValidateDecision` ([chaos/manager.go](file:///Users/zhengningdai/workspace/skyold/nofx/chaos/manager.go))。
    - 在此处根据账户权益和风险配置计算 **`PositionSizeUSD`** (仓位金额)。
    - 这是 AI 决策转化为可执行指令的关键一步。

## 3. 结果处理与执行 (Execution)

**文件**: [trader/auto_trader_chaos.go](file:///Users/zhengningdai/workspace/skyold/nofx/trader/auto_trader_chaos.go)

### `processChaosResult(result, ctx)`
...
- **逻辑**:
    - 读取 `Decision` 中的 `PositionSizeUSD`。
    - 获取当前市价，计算 `Quantity` (数量 = 金额 / 价格)。
    - 调用 `e.trader.OpenLong` / `OpenShort` 等接口下单。
