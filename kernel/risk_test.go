...
func TestRiskRCalculation(t *testing.T) {
	...
	err := validateDecision(&d1, accountEquity, 10, 10, 10.0, 10.0, baseRiskPercent)
	if err != nil {
	...
	err = validateDecision(&d2, accountEquity, 10, 10, 10.0, 10.0, baseRiskPercent)
	if err != nil {
		t.Errorf("Validation failed: %v", err)
	}
