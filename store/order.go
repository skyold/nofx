...
type TraderOrder struct {
	...
	FilledAt          UnixTime `gorm:"column:filled_at" json:"filled_at"`   // Unix milliseconds UTC
}
...
func (s *OrderStore) UpdateOrderStatus(id int64, status string, filledQty, avgPrice, commission float64) error {
	...
	if status == "FILLED" {
		updates["filled_at"] = UnixTime(time.Now().UTC().UnixMilli())
	}
...
func (s *OrderStore) GetLastFillTimeByExchange(exchangeID string) (int64, error) {
	...
	return int64(fill.CreatedAt), nil
}
...
// ClearHistory clears all transaction history for a specific trader
func (s *OrderStore) ClearHistory(traderID string) error {
	// Start a transaction
	return s.db.Transaction(func(tx *gorm.DB) error {
		// Delete fills
		if err := tx.Where("trader_id = ?", traderID).Delete(&TraderFill{}).Error; err != nil {
			return fmt.Errorf("failed to delete fills: %w", err)
		}

		// Delete orders
		if err := tx.Where("trader_id = ?", traderID).Delete(&TraderOrder{}).Error; err != nil {
			return fmt.Errorf("failed to delete orders: %w", err)
		}

		// Also clear positions? Usually "Clear History" implies trades/orders.
		// Positions are state, clearing them might be dangerous if they are open on exchange.
		// But if we reset history, we probably want to reset positions too?
		// The prompt says "Clear all historical transaction records ... reset history".
		// I'll stick to orders and fills for now. Resetting positions is handled by Rebuild/Sync.
